...
const tableDelete = (tariffTitle) => {
  let col_name = document.querySelectorAll('.col.name h3')
  col_name.forEach(item => {
    let col_title = item.innerHTML
    
  })
  console.log(col_name)
  delete_button.innerHTML = `
    <div class="top">
      <h3>Удалить тариф ${tariffTitle}</h3>
    </div>
    <div class="bot">
      <input type="button" value="Закрыть" class="btn dark" onclick="modalClose()" />
      <input type="button" value="Удалить" class="btn main" onclick="addTariff()" />
    </div>
  `;
